...
# Setup
###########
with open('testjson.json', 'r') as filehandle:
...
class car_control:
    ...
    def jsonreadout(self):
        print(randint(0,100),"Frame Width: ",FRAME_WIDTH,"\nMixing Rate: ",MIXING_RATE,"\nThrottle_cut_off_angle: ",THROTTLE_CUT_OFF_ANGLE,
        "\nThrottle_cut_off_rate: ",THROTTLE_CUT_OFF_RATE,"\nThrottle_Gain: ",THROTTLE_GAIN,"\nThrottle_Offset: ",THROTTLE_OFFSET,
        "\nThrottle P Gain: ",THROTTLE_P_GAIN,"\nThrottle I Gain: ",THROTTLE_I_GAIN,"\nThrottle I Min: ",THROTTLE_I_MIN,
        "\nThrottle I Max: ",THROTTLE_I_MAX,"\nThrottle D Gain: ",THROTTLE_D_GAIN,"\nSteering Offset: ",STEERING_OFFSET,
        "\nSteering P Gain: ",STEERING_P_GAIN,"\nSteering I Gain: ",STEERING_I_GAIN,"\nSteering I Min: ",STEERING_I_MIN,
        "\nSteering I Max: ",STEERING_I_MAX,"\nSteering D Gain: ",STEERING_D_GAIN,"\nThrottle Servo Min: ",THROTTLE_SERVO_MIN,
        "\nThrottle Servo Max: ",THROTTLE_SERVO_MAX,"\nSteering Servo Min: ",STEERING_SERVO_MIN,"\nSteering Servo Max: ",STEERING_SERVO_MAX, file=self.vari_file, flush=True)
    ...
    def update(self, line):
        ...
        if not self.paused.state:
            try:
                ...
                if data == 'PAUSE':
                    print('Pausing after receiving \'PAUSE\' from client', file=self.log_file, flush=True)
        ...
        if self.paused.state:
            ...
            try:
                ...
                data = client.recv(1024).decode()
                if data == 'UNPAUSE':
                    print('Unpausing after receiving \'UNPAUSE\' from client', file=self.log_file, flush=True)
                    self.paused.state = not self.paused.state
                    client.send(data.encode())
                elif data == 'LOAD':
                    ...
                    print('JSONREADOUT triggered by client', file=self.log_file, flush=True)
                    self.jsonreadout()
